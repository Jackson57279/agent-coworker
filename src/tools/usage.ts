      ...
      switch (action) {
        ...
        case "budget": {
          ...
          if (status.warnAtUsd !== null) {
            lines.push(
              `  Warning threshold:  ${formatCost(status.warnAtUsd)}${status.warningTriggered ? " ⚠️  TRIGGERED" : ""}`,
            );
          }
          if (status.stopAtUsd !== null) {
            lines.push(
              `  Hard-stop threshold: ${formatCost(status.stopAtUsd)}${status.stopTriggered ? " 🛑 EXCEEDED" : ""}`,
            );
          }
        ...
        case "set_budget": {
          ...
          if (
            typeof stopAtUsd === "number" &&
            typeof warnAtUsd === "number" &&
            warnAtUsd >= stopAtUsd
          ) {
          ...
          if (warnAtUsd !== undefined) {
            parts.push(
              warnAtUsd === null ? "  Warning cleared" : `  Warning at: ${formatCost(warnAtUsd)}`,
            );
          }
          if (stopAtUsd !== undefined) {
            parts.push(
              stopAtUsd === null
                ? "  Hard stop cleared"
                : `  Hard stop at: ${formatCost(stopAtUsd)}`,
            );
          }
        ...
        case "pricing": {
          ...
          const lines: string[] = [
            ...
            "",
            "Provider         | Model                                      | Input    | Output   | Cached | Long context",
            "─────────────────|────────────────────────────────────────────|──────────|──────────|────────|─────────────",
          ];
          ...
          for (const entry of catalog) {
            ...
            const longContext =
              ...
              entry.pricing.longContextOutputPerMillion !== undefined
                ...
                : "n/a";
            lines.push(
              `${entry.provider.padEnd(17)}| ${entry.model.padEnd(43)}| $${entry.pricing.inputPerMillion.toFixed(3).padEnd(7)}| $${entry.pricing.outputPerMillion.toFixed(3).padEnd(7)}| ${cached.padEnd(7)}| ${longContext}`,
            );
          }
